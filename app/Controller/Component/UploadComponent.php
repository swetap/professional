<?php
	...
	function shutdown(&$controller)
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL,$oldfile = NULL,$video=NULL) {
			...
			if($video)
			      $allowed=array('wmv','dat','mpeg','WMV','DAT','MPEG','mp4','MP4');
			...
			if (!is_null($allowed)) { $this->_allowed = array('wmv','dat','mpeg','WMV','DAT','MPEG','kml','KML','mp4','MP4'); } else { $this->_allowed = array('jpg','jpeg','gif','png','JPG','JPEG','GIF','PNG'); }
		...
		function uploadlong($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL,$oldfile = NULL,$video=NULL)
		{
			$this->result = false;
			$this->error  = false;
			...
			if($video)
			      $allowed=array('wmv','dat','mpeg','WMV','DAT','MPEG');
			...
			if (!is_null($allowed)) { $this->_allowed = array('wmv','dat','mpeg','WMV','DAT','MPEG','kml','KML'); } else { $this->_allowed = array('jpg','jpeg','gif','png','JPG','JPEG','GIF','PNG'); }
					...
					if (is_uploaded_file($fileTmp)) {
						...
						if ($rules == NULL) {
							...
							if (function_exists("imagecreatefromjpeg")) {
								...
								// -- handle it based on rules
								if (isset($rules['type']) && isset($rules['size'])) {
									$this->imagelong($this->_file, $rules['type'], $rules['size'], $rules['output'], $rules['quality']);
								} else {
									$this->error("Invalid \"rules\" parameter");
								}
							} else {
		...
		function image ($file, $type, $size, $output = NULL, $quality = NULL) {
			...
			if($width>375){
				...
				$type="resizetowidth";
				$width=(int)(($this->getWidth()*30)/100);
				$maxScale=$this->getWidth()+$width;
			}*/
			...
			switch ($type) {
				...
				case 'resizetowidth':
						$this->load($file['tmp_name']);
						$ratio = $maxScale /$this->getWidth();
				      	$height = $this->getheight() * $ratio;
			 		    $this->resize($maxScale,$height);
		...
		function imagelong($file, $type, $size, $output = NULL, $quality = NULL) {
